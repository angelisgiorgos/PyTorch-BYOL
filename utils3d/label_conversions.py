...
def convert_densepose_to_6part_lsp_labels(densepose_seg):
    """
    Convert 24 body-part labels (DensePose convention) to 6 body-part labels.
    """
    if isinstance(densepose_seg, torch.Tensor):
        lsp_6part_seg = torch.zeros_like(densepose_seg)
...
def convert_2Djoints_to_gaussian_heatmaps(joints2D, img_wh, std=4):
    ...
    size = int(2 * std ) # Truncate gaussian at 2 std from joint location.
