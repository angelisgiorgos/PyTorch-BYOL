...
def preprocess_densepose_UV(batch_size):
    DP_UV = loadmat(config.DP_UV_PROCESSED_FILE)
...
class TexturedIUVRenderer(nn.Module):
    def __init__(self,
        ...
        verts_uv_offset, verts_iuv, verts_map, faces_densepose = preprocess_densepose_UV(batch_size=batch_size)
        ...
        if projection_type == 'perspective':
            self.cameras = PerspectiveCameras(device=device,
                                              ...
                                              in_ndc=False # Used only for pytorch3d latest versions
                                              )
        ...
        if self.render_rgb:
            ...
            self.mesh_render = MeshRenderer(rasterizer=self.rasterizer,shader=self.rgb_shader)
    ...
    def forward(self, vertices, textures=None, cam_t=None, orthographic_scale=None, lights_rgb_settings=None,
        ...
        if self.render_rgb:
            if verts_features is not None:
                ...
                textures = torch.ones(vertices.shape[0], 1200, 800, 3, dtype=torch.float32, device=cam_t.device) * 0.7
                # textures = np.load(config.VERTEX_TEXTURE_FILE)
                # textures = torch.from_numpy(textures).cuda().float()
        ...
        if self.render_rgb:
            ...
            mesh_img = self.mesh_render(meshes_rgb.clone(), T=self.cameras.T)
            output['mesh_img'] = torch.clamp(mesh_img, max=1.0)
